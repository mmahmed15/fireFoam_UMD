...
const turbulentTemperatureRadiationCoupledQcWallFunctionMixedSTFvPatchScalarField::filmModelType&
turbulentTemperatureRadiationCoupledQcWallFunctionMixedSTFvPatchScalarField::
filmModel() const
{
...
turbulentTemperatureRadiationCoupledQcWallFunctionMixedSTFvPatchScalarField
(
    const turbulentTemperatureRadiationCoupledQcWallFunctionMixedSTFvPatchScalarField& psf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
//kvm                << "     predicted wallT [K] : " << gAverage(Twall) << nl
	...
	//const bool foundBlockageFactor = db().foundObject<volScalarField>("blockFactor");
	//const bool foundQnet = db().foundObject<volScalarField>("Qnet");
	//if(foundBlockageFactor)
	//{
	    Info<<"RollPaper page blockage."<<endl;
	...
	//{
	    ...
	    //scalarField& qneatInternal = qheatBoundary.patchInternalField();
	//}
	    ...
	    nbrTotalFlux[i] = nbrTotalFlux[i]*(1.0-blockingInternal[i]);    //Ning
	    qheatBoundary[i] = -nbrTotalFlux[i];	    // Ning 
